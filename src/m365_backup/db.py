...
from sqlalchemy import (
    Table, Column, Integer, String, DateTime, LargeBinary, MetaData, Text, ForeignKey, UniqueConstraint
)
from sqlalchemy.orm import registry, sessionmaker
from sqlalchemy import create_engine

DATABASE_URL = os.environ.get("DATABASE_URL", "postgresql+asyncpg://postgres:postgres@db:5432/m365backup")
...
def store_snapshot(label: Optional[str], collected: List[Dict[str, Any]]):
    ...
    try:
        ...
        for item in collected:
            ...
            # check dedup
            q = session.execute(messages_table.select().where(messages_table.c.message_hash == mhash))
            if q.first():
                continue
            session.execute(messages_table.insert().values(
...
def list_snapshots(limit: int = 50):
    session = Session()
    try:
        res = session.execute(snapshots_table.select().order_by(snapshots_table.c.created_at.desc()).limit(limit))
...
def get_snapshot_messages(snapshot_id: int):
    session = Session()
    try:
        res = session.execute(messages_table.select().where(messages_table.c.snapshot_id == snapshot_id))
        return [dict(row) for row in res]
    finally:
        session.close()
